...
const statusConfig: Record<InvoiceStatus, { icon: React.ReactNode; label: string; color: string; bg: string }> = {
  ...
  PARTIAL: { icon: <AlertCircle className="h-5 w-5" />, label: "Kısmi Ödeme", color: "text-orange-700", bg: "bg-orange-100" },
    ...
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <Card className="p-5">
          ...
          <div className="text-sm space-y-1">
            <p className="font-medium">{invoice.billingName || invoice.user?.name}</p>
            {invoice.billingAddress && (
              <>
                <p className="text-muted-foreground">{invoice.billingAddress.line1}</p>
                {invoice.billingAddress.line2 && (
                  <p className="text-muted-foreground">{invoice.billingAddress.line2}</p>
                )}
                <p className="text-muted-foreground">
                  {invoice.billingAddress.city}, {invoice.billingAddress.postalCode}
                </p>
                <p className="text-muted-foreground">{invoice.billingAddress.country}</p>
              </>
            )}
            <p className="text-muted-foreground mt-2">{invoice.user?.email}</p>
          </div>
        </Card>
      </div>
          ...
          <table className="w-full">
            ...
            <tfoot>
              <tr className="border-b">
                ...
                <td className="p-4 text-right">{currencySymbol}{invoice.subtotal?.toFixed(2)}</td>
              </tr>
              {invoice.taxAmount > 0 && (
                <tr className="border-b">
                  ...
                  <td className="p-4 text-right">{currencySymbol}{invoice.taxAmount?.toFixed(2)}</td>
                </tr>
              )}
              {invoice.discount > 0 && (
                <tr className="border-b">
                  <td colSpan={3} className="p-4 text-right text-green-600">İndirim</td>
                  <td className="p-4 text-right text-green-600">-{currencySymbol}{invoice.discount?.toFixed(2)}</td>
                </tr>
              )}
              <tr className="bg-muted/50">
                <td colSpan={3} className="p-4 text-right font-semibold">Genel Toplam</td>
                <td className="p-4 text-right font-bold text-lg">{currencySymbol}{invoice.total?.toFixed(2)}</td>
              </tr>
            </tfoot>
          </table>
        </div>
