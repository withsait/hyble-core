...
export function TrustFloatingBar() {
  ...
  // Sayfa %30'dan fazla scroll edildiğinde görünür olsun
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 pb-4">
        <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-md rounded-full border border-slate-200 dark:border-slate-700 shadow-lg px-6 py-3 inline-flex items-center gap-6 pointer-events-auto mx-auto">
          {trustIndicators.map((item, index) => (
            ...
            >
              ...
              {index < trustIndicators.length - 1 && (
                <span className="w-px h-4 bg-slate-200 dark:bg-slate-700 ml-4 hidden md:block" />
              )}
            </div>
          ))}
        </div>
